...
fun ParentApp(
    ...
    Scaffold(
        ...
        Box(modifier = Modifier.padding(paddingValues)) {
            ...
            when (selectedTab) {
                0 -> ParentHome(userName = userName, onNavigateToTab = onNavigateToTab)
                1 -> ParentWellness(userName = userName, onNavigateToTab = onNavigateToTab)
                2 -> ParentMessages(userName = userName, onNavigateToTab = onNavigateToTab)
                3 -> CommunityResourcesScreen(userName = userName)
                4 -> WearableSyncScreen(userName = userName)
                5 -> ParentSettings(userName = userName, userEmail = userEmail, onLogout = onLogout, onNavigateToTab = onNavigateToTab)
            }
        }
    }
}
