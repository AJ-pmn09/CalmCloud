-- Production-Ready Sample Data Seeding Script (Simplified)
...
SELECT 
    ...
    END
FROM students s
CROSS JOIN generate_series(1, 25);  -- 25 check-ins per student
...
FROM students s
CROSS JOIN generate_series(1, 20);  -- 20 activity logs per student
...
FROM students s
CROSS JOIN generate_series(1, 28);  -- Daily wearable data
...
FROM students s
CROSS JOIN generate_series(1, 15);  -- 15 wellness scores per student
...
BEGIN
    ...
    INSERT INTO assistance_requests (student_id, parent_id, parent_name, message, urgency, status, created_at)
    VALUES (student_emma_id, parent_sarah_id, 'Sarah Thompson', 
            ...
            'normal', 'pending', CURRENT_DATE - 2);
    
    -- Scenario 2: In-progress request (multiple children, high priority)
    INSERT INTO assistance_requests (student_id, parent_id, parent_name, message, urgency, status, handled_by, handled_by_name, handled_at, created_at)
    VALUES (student_ethan_id, parent_david_id, 'David Wilson', 
            ...
            'high', 'in-progress', associate_michael_id, 'Michael Johnson', CURRENT_DATE - 1, CURRENT_DATE - 5)
    ...
    INSERT INTO assistance_requests (student_id, parent_id, parent_name, message, urgency, status, handled_by, handled_by_name, handled_at, notes, created_at, updated_at)
    VALUES (student_james_id, parent_maria_id, 'Maria Martinez', 
            ...
            'low', 'resolved', associate_jennifer_id, 'Jennifer Parker', CURRENT_DATE - 10, 
    ...
    VALUES 
        (student_sophia_id, (SELECT id FROM users WHERE email = 'wei.chen@email.com'), 'Wei Chen',
         'Sophia has been very quiet lately. Concerned about her emotional well-being.', 'normal', 'pending', CURRENT_DATE - 3),
        (student_michael_id, (SELECT id FROM users WHERE email = 'robert.anderson@email.com'), 'Robert Anderson',
         'Michael needs support with time management and organization skills.', 'normal', 'pending', CURRENT_DATE - 1),
        (student_noah_id, (SELECT id FROM users WHERE email = 'jennifer.brown@email.com'), 'Jennifer Brown',
         'Noah seems to be having difficulty focusing in class. Requesting assessment.', 'high', 'pending', CURRENT_DATE - 4);
    ...
    VALUES 
        (student_ava_id, (SELECT id FROM users WHERE email = 'christopher.davis@email.com'), 'Christopher Davis',
         'Ava has been experiencing sleep issues. Need recommendations.', 'normal', 'resolved',
        ...
        (student_liam_id, (SELECT id FROM users WHERE email = 'patricia.garcia@email.com'), 'Patricia Garcia',
         'Liam needs support with peer relationships.', 'low', 'resolved',
...
BEGIN
    ...
    INSERT INTO emergency_alerts (student_id, alert_type, message, status, acknowledged_by, acknowledged_at, created_at)
    VALUES (student_james_id, 'urgent', 'Student expressed thoughts of self-harm. Crisis intervention initiated.', 'acknowledged', 
            associate_jennifer_id, CURRENT_TIMESTAMP - INTERVAL '30 minutes', CURRENT_TIMESTAMP - INTERVAL '1 hour');
    
    -- Scenario 3: Resolved alert
    INSERT INTO emergency_alerts (student_id, alert_type, message, status, acknowledged_by, acknowledged_at, resolved_by, resolved_at, resolution_notes, created_at)
    VALUES (student_sophia_id, 'support', 'Student requested immediate counseling support due to family crisis.', 'resolved',
            ...
            'Provided crisis counseling. Connected with family resources. Follow-up scheduled.', CURRENT_TIMESTAMP - INTERVAL '3 days');
...
BEGIN
    ...
    SELECT u.id INTO user_emma_id FROM users u WHERE u.email = 'emma.thompson@horizon.edu';
    SELECT u.id INTO user_james_id FROM users u WHERE u.email = 'james.martinez@horizon.edu';
    SELECT u.id INTO user_sophia_id FROM users u WHERE u.email = 'sophia.chen@horizon.edu';
    ...
    VALUES 
        (student_emma_id, user_emma_id, 'checkin_completed', 'Daily check-in submitted', '{"emotion": "anxious", "stress_level": 7}', CURRENT_TIMESTAMP - INTERVAL '2 hours'),
        ...
        (student_james_id, user_james_id, 'checkin_completed', 'Daily check-in submitted', '{"emotion": "calm", "stress_level": 3}', CURRENT_TIMESTAMP - INTERVAL '3 hours'),
        ...
        (student_sophia_id, user_sophia_id, 'checkin_completed', 'Daily check-in submitted', '{"emotion": "okay", "stress_level": 5}', CURRENT_TIMESTAMP - INTERVAL '5 hours'),
...
SELECT 
    ...
    jsonb_build_object('action', 'performed', 'timestamp', CURRENT_TIMESTAMP::text),
    CURRENT_TIMESTAMP - (random() * INTERVAL '30 days')
FROM users u
